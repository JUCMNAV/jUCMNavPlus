...
import seg.jUCMNav.editparts.dynamicContextEvaluationViewEditparts.DynamicContextEvaluationViewEditPartFactory;
import seg.jUCMNav.editparts.dynamicContextEvaluationViewEditparts.DynamicContextEvaluationViewObject;
import seg.jUCMNav.editparts.dynamicContextEvaluationViewEditparts.DynamicContextEvaluationViewRootEditPart;
...
/**
 * 
 * The Dynamic Context Overall Evaluation view.
 * 
 * @author aprajita
 * 
 */
public class DynamicContextEvaluationView  extends ViewPart implements IPartListener2, ISelectionChangedListener, ISelectionListener, JUCMNavRefreshableView {
    ...
    public void partActivated(IWorkbenchPartReference partRef) {
        if (partRef.getPart(false) == this || partRef.getPart(false) instanceof UCMNavMultiPageEditor ||
        		EvaluationStrategyManager.getInstance().getEvaluationAlgorithm().getEvaluationType() == IGRLStrategyAlgorithm.EVAL_TIMED) {
            setEditor(partRef);
        } else {
            // bug 709 - if we are no longer selecting a UCM editor, flush the current selection.
            if (isViewerAlive() && !(partRef.getPage().getActiveEditor() instanceof UCMNavMultiPageEditor)) {
    ...
    protected void handleSelectionChanged(IWorkbenchPart part, ISelection selection) {
        ...
        if (EvaluationStrategyManager.getInstance().getEvaluationStrategy() != null &&
            ...
            if (sel.size() > 0) {
                ...
                if (obj != null && obj instanceof EditPart && !(obj instanceof DynamicContextEvaluationViewRootEditPart)) {
                    if (viewer.getContents() != null && viewer.getContents().getModel() != null) {
                        if ((viewer.getContents().getModel()).equals(((EditPart) obj).getModel())) {
                            return;
                        }
                        //Keep the evaluation of last timepoint group, when anything other than the TGGroup or, DynamicContext is selected
                        if(!(((EditPart) obj).getModel() instanceof TimepointGroup || ((EditPart) obj).getModel() instanceof DynamicContext || 
                        		((EditPart) obj).getModel() instanceof DynamicContextEvaluationViewObject))
                        	return;
    ...
    private void setEditor(UCMNavMultiPageEditor editor) {
        viewer.setEditDomain(new UrnEditDomain(null));
        viewer.setEditPartFactory(new DynamicContextEvaluationViewEditPartFactory());

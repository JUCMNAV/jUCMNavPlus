...
public class AttachNewExtremitiesToStubCommand extends CompoundCommand {
    ...
    /**
     * An empty compound means there were no new extremities to attach, and undoing nothing always
     * succeeds.
     *
     * GEF's CompoundCommand returns false from canUndo() when its command list is empty. Because
     * this command is nested unconditionally by RefactorIntoStubCommand, that false propagated to
     * the enclosing command, whose canUndo() then reported false as well. CommandStack.undo()
     * opens with `if (!canUndo()) return;`, so the whole refactor could never be undone (legacy
     * bug 923, #28). canExecute() above is overridden for exactly the same reason.
     */

...
| **10.0.2 — OR-fork branch probabilities** | The Condition Editor (double-click an OR-fork) now edits each branch's **probability** alongside its condition, backed by undo/redo, with a warning when the branch probabilities don't sum to 1.0; the same sum check is available on a dynamic stub's plug-in probabilities. Both checks stay hidden unless probabilities are actually in use (every branch defaults to 1.0). **Static slicing** now runs off the UI thread under a cancellable progress dialog instead of synchronously on the UI thread. The JUnit **test suite was migrated from JUnit 3 to JUnit 4** (POJO test classes + a shared `JUCMNavTestFixture`). Also: a post-dispose crash guard for the outline page when closing an editor tab | Set and review branch probabilities without digging into the Advanced properties tab, and get told when they don't add up — without nagging the majority of models that don't use probabilities at all; slicing a large model no longer freezes the workbench (and you can cancel it); contributors get the cleaner, modern test style |
| **10.0.3 — crash & menu fixes** | **Selecting a start/end point with no condition** no longer throws: `AddConditionLabelAction` had an operator-precedence bug (`a != null && b == null \|\| b.length() == 0`) that dereferenced a null pre/postcondition on every selection change. Because action enablement is refreshed in one unprotected loop, that single exception also **silently emptied context menus** of every action registered after it — which is why **"Run All Scenarios"** vanished from the Scenarios/Strategies view; the refresh loop is now failure-isolated per action. **Dynamic stub dialog**: the Edit-condition wizard is parented on the (modal) stub dialog, so it opens *on top* instead of forcing you to close the stub window first, and unchecking a plug-in map no longer NPEs on partial/pointcut bindings. **Teardown crashes**: the Elements view no longer throws "Need an underlying widget…" when closing the workbench, plus matching disposed-control guards across the KPI, Strategies, Dynamic Contexts and Elements views (GEF's own tree `setContents` checks the widget for null but not for disposed) | Clicking an end point, closing the workbench, or editing a dynamic stub's condition no longer produces error-log entries; menu items that quietly disappeared are back, and one misbehaving action can no longer take a whole context menu down with it |
